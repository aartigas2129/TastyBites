...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  $fileName = time() . "_" . basename($_FILES["recipeImage"]["name"]); // unique filename
  $targetFilePath = $targetDir . $fileName;

  if (move_uploaded_file($_FILES["recipeImage"]["tmp_name"], $targetFilePath)) {
    $sql = "INSERT INTO recipes (name, description, ingredients, instructions, image) 
...
<head>
  ...
  <link href="https://fonts.googleapis.com/css2?family=Felipa&family=Poppins:wght@400;500;600&display=swap" rel="stylesheet">
...
<nav class="navbar navbar-expand-lg shadow-sm">
  <div class="container-fluid">
    <!-- Brand -->
    <a class="navbar-brand" href="#">TastyBites</a>

    <!-- Search -->
    <form class="d-flex ms-3 flex-grow-1">
      <div class="input-group search-group w-50">
        <span class="input-group-text search-icon">🔍</span>
        <input class="form-control search-input" type="search" placeholder="Search recipes..." aria-label="Search">
      </div>
    </form>

    <!-- Navbar links aligned wide -->
    <ul class="navbar-nav ms-4">
      <li class="nav-item"><a href="#" class="nav-link">Home</a></li>
      <li class="nav-item"><a href="#" class="nav-link">My Recipes</a></li>
      <li class="nav-item"><a href="#" class="nav-link">Add Recipe</a></li>
      <li class="nav-item"><span class="nav-link">Hello, Aiken!</span></li>
    </ul>
  </div>
</nav>
...
<!-- Hero Carousel -->
<div id="heroCarousel" class="carousel slide container py-4" data-bs-ride="carousel">
  <div class="carousel-inner">

    <?php 
    $first = true;
    $carousel = $conn->query("SELECT * FROM recipes ORDER BY id DESC LIMIT 2"); // get 2 latest recipes
